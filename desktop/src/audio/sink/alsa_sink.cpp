...
Result<SinkConfig> AlsaSink::open(const SinkConfig& requested) {
    ...
    if (snd_pcm_set_params(handle_, SND_PCM_FORMAT_FLOAT_LE, SND_PCM_ACCESS_RW_INTERLEAVED,
                           requested.format.channels, requested.format.sample_rate, 1, 500000) < 0) {
        close();
        return err(ErrorCode::DeviceFormatUnsupported, "The ALSA device rejected the PCM format.");
    }
    config_ = requested;
    opened_ = true;
    return config_;
...
Status AlsaSink::start() {
    if (!opened_ || handle_ == nullptr) return err(ErrorCode::InvalidState, "The ALSA sink is not open.");
    return ok();
}
...
Status AlsaSink::write(const PlanarFrames frames) noexcept {
    ...
    while (written < frames.frames) {
        const auto result = snd_pcm_writei(handle_, interleaved.data() + written * frames.channels,
                                           frames.frames - written);
        if (result == -EPIPE) {
            snd_pcm_prepare(handle_);
            return err(ErrorCode::BufferUnderrun, "The ALSA playback buffer underrun occurred.");
        }
        if (result < 0) return err(ErrorCode::DeviceLost, "The ALSA device stopped accepting audio.");
