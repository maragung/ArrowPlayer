...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    ...
    while (pos < input.size()) {
        ...
        (void)arrow::text::decode_utf8(input, pos);
        if (pos <= before) {
    ...
    if (codepoints != arrow::text::utf8_length(input)) {
        fail("utf8_length disagrees with a decode walk of the same input");
    }

    // 2. Sanitisation is total: valid UTF-8 out, for any bytes in.
    const std::string clean = arrow::text::sanitize_utf8(input);
    if (!arrow::text::is_valid_utf8(clean)) {
    ...
    for (std::size_t n = 0; n <= codepoints + 1; ++n) {
        const std::size_t offset = arrow::text::utf8_offset_of(clean, n);
    ...
    if (!arrow::text::is_valid_utf8(arrow::text::to_lower(clean)) ||
        !arrow::text::is_valid_utf8(arrow::text::to_upper(clean)) ||
        !arrow::text::is_valid_utf8(arrow::text::to_title(clean)) ||
        !arrow::text::is_valid_utf8(arrow::text::collapse_whitespace(clean)) ||
        !arrow::text::is_valid_utf8(arrow::text::sort_key(clean, "en"))) {
    ...
    if (codepoints > 0) {
        ...
        const std::string part = arrow::text::utf8_substr(clean, start, codepoints);
        if (!arrow::text::is_valid_utf8(part)) {
    ...
    if (arrow::text::normalize_relative_path(input, normalized)) {
        if (arrow::text::is_unsafe_relative_path(normalized)) {
