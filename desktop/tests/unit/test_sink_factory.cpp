...
TEST(SinkFactory, ExplicitNullCreatesNullSink) {
    const auto result = eclipse::audio::make_sink({.preference = eclipse::audio::SinkPreference::Null});
    ASSERT_TRUE(result);
...
TEST(SinkFactory, ExplicitAlsaReportsUnavailable) {
    const auto result = eclipse::audio::make_sink({.preference = eclipse::audio::SinkPreference::Alsa});
    EXPECT_FALSE(result);
    ASSERT_FALSE(result);
