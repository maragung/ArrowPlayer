...
TEST(SinkFactory, ExplicitNullCreatesNullSink) {
    const auto result =
        arrow::audio::make_sink({.preference = arrow::audio::SinkPreference::Null});
    ASSERT_TRUE(result);
    EXPECT_EQ(result.value()->device_name(), "null");
}

TEST(SinkFactory, AutomaticProvidesAUsableSink) {
    const auto result = arrow::audio::make_sink();
    ASSERT_TRUE(result);
...
TEST(SinkFactory, ExplicitAlsaReportsUnavailable) {
    const auto result =
        arrow::audio::make_sink({.preference = arrow::audio::SinkPreference::Alsa});
    EXPECT_FALSE(result);
    ASSERT_FALSE(result);
    EXPECT_EQ(result.error().code(), arrow::ErrorCode::DeviceNotFound);
}
#endif
