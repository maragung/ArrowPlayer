...
std::filesystem::path make_wav() {
    const auto path = std::filesystem::temp_directory_path() / "arrow-player-test.wav";
...
TEST(WavDecoder, RejectsInvalidStateAndMalformedInput) {
    arrow::audio::WavDecoder decoder;
    EXPECT_EQ(decoder.read({nullptr, 0, 1}).error().code(), arrow::ErrorCode::InvalidState);
    const auto path = std::filesystem::temp_directory_path() / "arrow-player-bad.wav";
    std::ofstream(path, std::ios::binary).write("bad", 3);
    const auto result = decoder.open(path);
    EXPECT_FALSE(result);
    EXPECT_EQ(result.error().code(), arrow::ErrorCode::CorruptStream);
    std::filesystem::remove(path);
}
