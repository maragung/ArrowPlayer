...
MainWindow::MainWindow(const ShellInfo& info, QWidget* parent) : QWidget(parent), info_(info) {
    setWindowTitle(tr("Eclipse Player"));
    ...
    QAction* about_action =
        help_menu->addAction(tr("&About Eclipse Player…"), this, &MainWindow::showAboutDialog);
...
QString MainWindow::aboutText() const {
    ...
    // would forbid.
    QString text = tr("Eclipse Player %1").arg(version);
    text += QLatin1Char('\n');
...
void MainWindow::showAboutDialog() {
    QDialog dialog(this);
    dialog.setWindowTitle(tr("About Eclipse Player"));
