...
//
// The ECLIPSE_SMOKE_TEST hook at the bottom is the Phase 0 answer to a
...
// embedded in the binary via the eclipse-i18n resource (ui/CMakeLists.txt).
...
bool install_translators(QCoreApplication& app, const QString& locale) {
    ...
    if (app_translator->load(QLocale(locale), QStringLiteral("eclipse"), QStringLiteral("_"),
                             QStringLiteral(":/i18n"))) {
...
int run_shell(int argc, char** argv, const ShellInfo& info) {
    ...
    app.setApplicationName(QStringLiteral("Eclipse Player"));
    ...
    if (qEnvironmentVariableIsSet("ECLIPSE_SMOKE_TEST")) {
